...
#ifndef _LIBCUDAFORDALICXX_ATOMIC_BASE_H
#define _LIBCUDAFORDALICXX_ATOMIC_BASE_H

#include "cxx_atomic.h"
#include <type_traits>

_LIBCUDAFORDALICXX_INLINE_VISIBILITY inline _LIBCUDAFORDALICXX_CONSTEXPR int __cxx_atomic_order_to_int(memory_order __order) {
...
_LIBCUDAFORDALICXX_INLINE_VISIBILITY inline _LIBCUDAFORDALICXX_CONSTEXPR int __cxx_atomic_failure_order_to_int(memory_order __order) {
  // Avoid switch statement to make this a constexpr.
  return __order == memory_order_relaxed ? __ATOMIC_RELAXED:
         (__order == memory_order_acquire ? __ATOMIC_ACQUIRE:
...
inline constexpr
 bool __cxx_atomic_is_lock_free(size_t __x) {
  #if defined(_LIBCUDAFORDALICXX_NO_RUNTIME_LOCK_FREE)
    return __x <= 8;
  #else
...
#endif // _LIBCUDAFORDALICXX_ATOMIC_BASE_H
