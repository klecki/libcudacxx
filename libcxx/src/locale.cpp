...
#include "clocale"
#include "cstring"
#if defined(_LIBCUDAFORDALICXX_MSVCRT)
#define _CTYPE_DISABLE_MACROS
#endif
#include "cwctype"
#include "__sso_allocator"
#if defined(_LIBCUDAFORDALICXX_MSVCRT) || defined(__MINGW32__)
...
_LIBCUDAFORDALICXX_NORETURN static void __throw_runtime_error(const string &msg)
{
#ifndef _LIBCUDAFORDALICXX_NO_EXCEPTIONS
...
class _LIBCUDAFORDALICXX_HIDDEN locale::__imp
    : public facet
{
    enum {N = 28};
...
{
#ifdef _LIBCUDAFORDALICXX_HAS_DEFAULTRUNELOCALE
    return isascii(c) ? _DefaultRuneLocale.__mapupper[c] : c;
#elif defined(__GLIBC__) || defined(__EMSCRIPTEN__) || \
      defined(__NetBSD__)
    return isascii(c) ? ctype<char>::__classic_upper_table()[c] : c;
#else
    return (isascii(c) && iswlower_l(c, _LIBCUDAFORDALICXX_GET_C_LOCALE)) ? c-L'a'+L'A' : c;
#endif
}
    ...
    for (; low != high; ++low)
      ...
      defined(__NetBSD__)
        *low = isascii(*low) ? ctype<char>::__classic_upper_table()[*low]
                             : *low;
#else
        *low = (isascii(*low) && islower_l(*low, _LIBCUDAFORDALICXX_GET_C_LOCALE)) ? (*low-L'a'+L'A') : *low;
#endif
    return low;
}
...
#else
    return (isascii(c) && isupper_l(c, _LIBCUDAFORDALICXX_GET_C_LOCALE)) ? c-L'A'+'a' : c;
#endif
}
    ...
    for (; low != high; ++low)
      ...
      defined(__NetBSD__)
        *low = isascii(*low) ? ctype<char>::__classic_lower_table()[*low]
                             : *low;
#else
        *low = (isascii(*low) && isupper_l(*low, _LIBCUDAFORDALICXX_GET_C_LOCALE)) ? *low-L'A'+L'a' : *low;
#endif
    return low;
}
...
#else
    return (isascii(c) && islower_l(c, _LIBCUDAFORDALICXX_GET_C_LOCALE)) ? c-'a'+'A' : c;
#endif
}
...
#else
        *low = (isascii(*low) && islower_l(*low, _LIBCUDAFORDALICXX_GET_C_LOCALE)) ? *low-'a'+'A' : *low;
#endif
    return low;
}
...
#else
    return (isascii(c) && isupper_l(c, _LIBCUDAFORDALICXX_GET_C_LOCALE)) ? c-'A'+'a' : c;
#endif
}
...
#else
        *low = (isascii(*low) && isupper_l(*low, _LIBCUDAFORDALICXX_GET_C_LOCALE)) ? *low-'A'+'a' : *low;
#endif
    return low;
}
...
#ifdef _LIBCUDAFORDALICXX_PROVIDES_DEFAULT_RUNE_TABLE
const ctype<char>::mask*
ctype<char>::classic_table()  _NOEXCEPT
{
    static _LIBCUDAFORDALICXX_CONSTEXPR const ctype<char>::mask builtin_table[table_size] = {
        cntrl,                          cntrl,
        cntrl,                          cntrl,
        cntrl,                          cntrl,
        cntrl,                          cntrl,
...
ctype<char>::classic_table()  _NOEXCEPT
{
    ...
    return _LIBCUDAFORDALICXX_GET_C_LOCALE->__ctype_b;
#elif __sun__
    return __ctype_mask;
#elif defined(_LIBCUDAFORDALICXX_MSVCRT) || defined(__MINGW32__)
...
{
    return _LIBCUDAFORDALICXX_GET_C_LOCALE->__ctype_tolower;
}

const int*
ctype<char>::__classic_upper_table() _NOEXCEPT
{
    return _LIBCUDAFORDALICXX_GET_C_LOCALE->__ctype_toupper;
}
#elif __NetBSD__
const short*
ctype<char>::__classic_lower_table() _NOEXCEPT
...
{
#ifdef _LIBCUDAFORDALICXX_WCTYPE_IS_MASK
        ...
        else
        {
            *vec = 0;
            ...
            if (iswspace_l(ch, __l))
                *vec |= space;
#ifndef _LIBCUDAFORDALICXX_CTYPE_MASK_IS_COMPOSITE_PRINT
            ...
            if (iswlower_l(ch, __l))
                *vec |= lower;
#ifndef _LIBCUDAFORDALICXX_CTYPE_MASK_IS_COMPOSITE_ALPHA
            ...
            if (iswpunct_l(ch, __l))
                *vec |= punct;
#ifndef _LIBCUDAFORDALICXX_CTYPE_MASK_IS_COMPOSITE_XDIGIT
...
codecvt<wchar_t, char, mbstate_t>::codecvt(size_t refs)
    : locale::facet(refs),
      __l(_LIBCUDAFORDALICXX_GET_C_LOCALE)
{
}
...
{
    if (__l != _LIBCUDAFORDALICXX_GET_C_LOCALE)
        freelocale(__l);
}
...
__codecvt_utf8<wchar_t>::do_out(state_type&,
    ...
    extern_type* to, extern_type* to_end, extern_type*& to_nxt) const
{
#if defined(_LIBCUDAFORDALICXX_SHORT_WCHAR)
    const uint16_t* _frm = reinterpret_cast<const uint16_t*>(frm);
    const uint16_t* _frm_end = reinterpret_cast<const uint16_t*>(frm_end);
    const uint16_t* _frm_nxt = _frm;
#else
...
struct _LIBCUDAFORDALICXX_HIDDEN __time_get_temp
...
{
    if (__loc_ != _LIBCUDAFORDALICXX_GET_C_LOCALE)
        freelocale(__loc_);
}
...
#if defined(_LIBCUDAFORDALICXX_MSVCRT) || defined(__MINGW32__)
    if (lc->p_sign_posn == 0)
#else // _LIBCUDAFORDALICXX_MSVCRT
    if (lc->int_p_sign_posn == 0)
#endif // !_LIBCUDAFORDALICXX_MSVCRT
        __positive_sign_ = "()";
    else
        __positive_sign_ = lc->positive_sign;
#if defined(_LIBCUDAFORDALICXX_MSVCRT) || defined(__MINGW32__)
...
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS collate<char>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS collate<wchar_t>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS num_get<char>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS num_get<wchar_t>;

template struct _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS __num_get<char>;
template struct _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS __num_get<wchar_t>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS num_put<char>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS num_put<wchar_t>;

template struct _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS __num_put<char>;
template struct _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS __num_put<wchar_t>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS time_get<char>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS time_get<wchar_t>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS time_get_byname<char>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS time_get_byname<wchar_t>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS time_put<char>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS time_put<wchar_t>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS time_put_byname<char>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS time_put_byname<wchar_t>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS moneypunct<char, false>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS moneypunct<char, true>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS moneypunct<wchar_t, false>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS moneypunct<wchar_t, true>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS moneypunct_byname<char, false>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS moneypunct_byname<char, true>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS moneypunct_byname<wchar_t, false>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS moneypunct_byname<wchar_t, true>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS money_get<char>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS money_get<wchar_t>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS __money_get<char>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS __money_get<wchar_t>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS money_put<char>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS money_put<wchar_t>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS __money_put<char>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS __money_put<wchar_t>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS messages<char>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS messages<wchar_t>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS messages_byname<char>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS messages_byname<wchar_t>;

template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS codecvt_byname<char, char, mbstate_t>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS codecvt_byname<wchar_t, char, mbstate_t>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS codecvt_byname<char16_t, char, mbstate_t>;
template class _LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS codecvt_byname<char32_t, char, mbstate_t>;
