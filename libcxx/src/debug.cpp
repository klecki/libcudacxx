...
#include "mutex"
#if defined(__unix__) && !defined(__ANDROID__) && defined(__ELF__) && defined(_LIBCUDAFORDALICXX_HAS_COMMENT_LIB_PRAGMA)
...
std::string __libcpp_debug_info::what() const {
  string msg = __file_;
  msg += ":" + to_string(__line_) + ": _LIBCUDAFORDALICXX_ASSERT '";
  msg += __pred_;
  msg += "' failed. ";
  msg += __msg_;
  return msg;
}
_LIBCUDAFORDALICXX_NORETURN void __libcpp_abort_debug_function(__libcpp_debug_info const& info) {
    std::fprintf(stderr, "%s\n", info.what().c_str());
    std::abort();
}

_LIBCUDAFORDALICXX_SAFE_STATIC __libcpp_debug_function_type
...
__get_db()
{
    static _LIBCUDAFORDALICXX_NO_DESTROY __libcpp_db db;
    return &db;
}
...
mut()
{
    static _LIBCUDAFORDALICXX_NO_DESTROY mutex_type m;
    return m;
}
...
__libcpp_db::__find_c_from_i(void* __i) const
{
    ...
    _LIBCUDAFORDALICXX_ASSERT(i != nullptr, "iterator not found in debug database.");
...
__libcpp_db::__find_c(void* __c) const
{
    size_t hc = hash<void*>()(__c) % static_cast<size_t>(__cend_ - __cbeg_);
    __c_node* p = __cbeg_[hc];
    _LIBCUDAFORDALICXX_ASSERT(p != nullptr, "debug mode internal logic error __find_c A");
    while (p->__c_ != __c)
    {
        p = p->__next_;
        _LIBCUDAFORDALICXX_ASSERT(p != nullptr, "debug mode internal logic error __find_c B");
    }
    return p;
}
...
__libcpp_db::__erase_c(void* __c)
{
    ...
    if (__cend_ != __cbeg_)
    {
        ...
        if (p == nullptr)
            return;
        __c_node* q = nullptr;
        _LIBCUDAFORDALICXX_ASSERT(p != nullptr, "debug mode internal logic error __erase_c A");
        while (p->__c_ != __c)
        {
            q = p;
            p = p->__next_;
            if (p == nullptr)
                return;
            _LIBCUDAFORDALICXX_ASSERT(p != nullptr, "debug mode internal logic error __erase_c B");
        }
        if (q == nullptr)
...
__libcpp_db::swap(void* c1, void* c2)
{
    ...
    _LIBCUDAFORDALICXX_ASSERT(p1 != nullptr, "debug mode internal logic error swap A");
    while (p1->__c_ != c1)
    {
        p1 = p1->__next_;
        _LIBCUDAFORDALICXX_ASSERT(p1 != nullptr, "debug mode internal logic error swap B");
    }
    hc = hash<void*>()(c2) % static_cast<size_t>(__cend_ - __cbeg_);
    __c_node* p2 = __cbeg_[hc];
    _LIBCUDAFORDALICXX_ASSERT(p2 != nullptr, "debug mode internal logic error swap C");
    while (p2->__c_ != c2)
    {
        p2 = p2->__next_;
        _LIBCUDAFORDALICXX_ASSERT(p2 != nullptr, "debug mode internal logic error swap D");
    }
...
__c_node::__remove(__i_node* p)
{
    ...
    _LIBCUDAFORDALICXX_ASSERT(r != end_, "debug mode internal logic error __c_node::__remove");
    if (--end_ != r)
