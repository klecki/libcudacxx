...
thread::join()
{
    ...
    if (!__libcpp_thread_isnull(&__t_))
    {
        ...
        if (ec == 0)
            __t_ = _LIBCUDAFORDALICXX_NULL_THREAD;
    }
...
#   if defined(_LIBCUDAFORDALICXX_WARNING)
        _LIBCUDAFORDALICXX_WARNING("hardware_concurrency not yet implemented")
#   else
...
template <class T>
class _LIBCUDAFORDALICXX_HIDDEN __hidden_allocator
{
public:
...
class _LIBCUDAFORDALICXX_HIDDEN __thread_struct_imp
{
