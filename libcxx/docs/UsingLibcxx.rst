...
**_LIBCUDAFORDALICXX_ENABLE_THREAD_SAFETY_ANNOTATIONS**:
  This macro is used to enable -Wthread-safety annotations on libc++'s
  ``std::mutex`` and ``std::lock_guard``. By default these annotations are
  disabled and must be manually enabled by the user.

**_LIBCUDAFORDALICXX_DISABLE_VISIBILITY_ANNOTATIONS**:
  This macro is used to disable all visibility annotations inside libc++.
  Defining this macro and then building libc++ with hidden visibility gives a
  build of libc++ which does not export any symbols, which can be useful when
  building statically for inclusion into another library.

**_LIBCUDAFORDALICXX_DISABLE_EXTERN_TEMPLATE**:
...
**_LIBCUDAFORDALICXX_ENABLE_TUPLE_IMPLICIT_REDUCED_ARITY_EXTENSION**:
...
**_LIBCUDAFORDALICXX_DISABLE_ADDITIONAL_DIAGNOSTICS**:
...
**_LIBCUDAFORDALICXX_NO_VCRUNTIME**:
  ...
  or they may clash with other headers. The `_LIBCUDAFORDALICXX_NO_VCRUNTIME` macro
...
**_LIBCUDAFORDALICXX_ENABLE_NODISCARD**:
...
**_LIBCUDAFORDALICXX_DISABLE_NODISCARD_EXT**:
...
**_LIBCUDAFORDALICXX_DISABLE_DEPRECATION_WARNINGS**:
...
-----------------------------------
**_LIBCUDAFORDALICXX_ENABLE_CXX17_REMOVED_FEATURES**:
  This macro is used to re-enable all the features removed in C++17. The effect
  is equivalent to manually defining each macro listed below.

**_LIBCUDAFORDALICXX_ENABLE_CXX17_REMOVED_UNEXPECTED_FUNCTIONS**:
  This macro is used to re-enable the `set_unexpected`, `get_unexpected`, and
  `unexpected` functions, which were removed in C++17.

**_LIBCUDAFORDALICXX_ENABLE_CXX17_REMOVED_AUTO_PTR**:
  This macro is used to re-enable `std::auto_ptr` in C++17.

C++2a Specific Configuration Macros:
------------------------------------
**_LIBCUDAFORDALICXX_DISABLE_NODISCARD_AFTER_CXX17**:
...
extension must be enabled by defining ``_LIBCUDAFORDALICXX_ENABLE_NODISCARD``. The extended
...
example ``_LIBCUDAFORDALICXX_DISABLE_NODISCARD_AFTER_CXX17``.

Applications of the second form, which are pure extensions, may be disabled
by defining ``_LIBCUDAFORDALICXX_DISABLE_NODISCARD_EXT``.


Entities declared with ``_LIBCUDAFORDALICXX_NODISCARD_EXT``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
