...
**_LIBCUDAFORDALICXX_OVERRIDABLE_FUNC_VIS**
  Mark a symbol as being exported by the libc++ library, but allow it to be
  overridden locally. On non-Windows, this is equivalent to `_LIBCUDAFORDALICXX_FUNC_VIS`.
...
**_LIBCUDAFORDALICXX_HIDE_FROM_ABI**
  Mark a function as not being part of the ABI of any final linked image that
  uses it.

**_LIBCUDAFORDALICXX_HIDE_FROM_ABI_AFTER_V1**
  Mark a function as being hidden from the ABI (per `_LIBCUDAFORDALICXX_HIDE_FROM_ABI`)
  ...
  ABI, we should create a new _LIBCUDAFORDALICXX_HIDE_FROM_ABI_AFTER_XXX macro, and we can
  use it to start removing symbols from the ABI after that stable version.

**_LIBCUDAFORDALICXX_HIDE_FROM_ABI_PER_TU**
  This macro controls whether symbols hidden from the ABI with `_LIBCUDAFORDALICXX_HIDE_FROM_ABI`
  ...
  together. This also means that functions marked with `_LIBCUDAFORDALICXX_HIDE_FROM_ABI`
  ...
  `_LIBCUDAFORDALICXX_HIDE_FROM_ABI_PER_TU_BY_DEFAULT` is defined, which is intended to
  be used by vendors only (see below).

**_LIBCUDAFORDALICXX_HIDE_FROM_ABI_PER_TU_BY_DEFAULT**
  This macro controls the default value for `_LIBCUDAFORDALICXX_HIDE_FROM_ABI_PER_TU`.
  When the macro is defined, per TU ABI insulation is enabled by default, and
  `_LIBCUDAFORDALICXX_HIDE_FROM_ABI_PER_TU` is defined to 1 unless overridden by users.
  Otherwise, per TU ABI insulation is disabled by default, and
  `_LIBCUDAFORDALICXX_HIDE_FROM_ABI_PER_TU` is defined to 0 unless overridden by users.
  ...
  control the behavior they want by defining `_LIBCUDAFORDALICXX_HIDE_FROM_ABI_PER_TU`
  appropriately.
...
**_LIBCUDAFORDALICXX_TYPE_VIS**
  Mark a type's typeinfo, vtable and members as having default visibility.
  This attribute cannot be used on class templates.

**_LIBCUDAFORDALICXX_TEMPLATE_VIS**
...
**_LIBCUDAFORDALICXX_EXTERN_TEMPLATE_TYPE_VIS**
  Mark the member functions, typeinfo, and vtable of the type named in
  a `_LIBCUDAFORDALICXX_EXTERN_TEMPLATE` declaration as being exported by the libc++ library.
  This attribute must be specified on all extern class template declarations.

  This macro is used to override the `_LIBCUDAFORDALICXX_TEMPLATE_VIS` attribute
  specified on the primary template and to export the member functions produced
  by the explicit instantiation in the dylib.

  **GCC Behavior**: GCC ignores visibility attributes applied the type in
  extern template declarations and applying an attribute results in a warning.
  However since `_LIBCUDAFORDALICXX_TEMPLATE_VIS` is the same as
  ...
  **Windows Behavior**: `extern template` and `dllexport` are fundamentally
  ...
  extern template declaration (i.e. using `_LIBCUDAFORDALICXX_FUNC_VIS`) is the correct
  thing to do for them.

**_LIBCUDAFORDALICXX_CLASS_TEMPLATE_INSTANTIATION_VIS**
...
**_LIBCUDAFORDALICXX_METHOD_TEMPLATE_IMPLICIT_INSTANTIATION_VIS**
  ...
  `_LIBCUDAFORDALICXX_TYPE_VIS` or classes with an extern template instantiation
  declaration marked with `_LIBCUDAFORDALICXX_EXTERN_TEMPLATE_TYPE_VIS`.

  When building libc++ with hidden visibility, we want explicit template
  instantiations to export members, which is consistent with existing Windows
  behavior. We also want classes annotated with `_LIBCUDAFORDALICXX_TYPE_VIS` to export
  ...
  either marked `_LIBCUDAFORDALICXX_TYPE_VIS` or with extern template instantiation
  declarations marked with `_LIBCUDAFORDALICXX_EXTERN_TEMPLATE_TYPE_VIS` also get default
  ...
  against the libc++ headers after making `_LIBCUDAFORDALICXX_TYPE_VIS` and
  `_LIBCUDAFORDALICXX_EXTERN_TEMPLATE_TYPE_VIS` expand to default visibility.

**_LIBCUDAFORDALICXX_EXCEPTION_ABI**
...
**_LIBCUDAFORDALICXX_INTERNAL_LINKAGE**
...
**_LIBCUDAFORDALICXX_ALWAYS_INLINE**
