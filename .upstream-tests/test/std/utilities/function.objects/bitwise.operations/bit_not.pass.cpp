...
int main(int, char**)
{
    typedef cuda_for_dali::std::bit_not<int> F;
    const F f = F();
    ...
    typedef cuda_for_dali::std::bit_not<> F2;
    const F2 f2 = F2();
    ...
    constexpr int foo = cuda_for_dali::std::bit_not<int> () (0xEA95) & 0xFFFF;
    static_assert ( foo == 0x156A, "" );

    constexpr int bar = cuda_for_dali::std::bit_not<> () (0xEA95) & 0xFFFF;
