...
{
    {
    typedef cuda_for_dali::std::bit_xor<int> F;
    const F f = F();
...
#if TEST_STD_VER > 11
    {
    typedef cuda_for_dali::std::bit_xor<> F2;
    const F2 f = F2();
    assert(f(0xEA95, 0xEA95) == 0);
    ...
    constexpr int foo = cuda_for_dali::std::bit_xor<int> () (0x58D3, 0xEA95);
    static_assert ( foo == 0xB246, "" );

    constexpr int bar = cuda_for_dali::std::bit_xor<> () (0x58D3L, 0xEA95);
