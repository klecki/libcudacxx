...
int main(int, char**)
{
    typedef cuda_for_dali::std::less<int> F;
    const F f = F();
...
#ifndef __CUDA_ARCH__
    {
        ...
        do_pointer_comparison_test<int, cuda_for_dali::std::less>();
    }
#endif
#if TEST_STD_VER > 11
    typedef cuda_for_dali::std::less<> F2;
    const F2 f2 = F2();
    assert(!f2(36, 36));
    assert(!f2(36, 6));
    assert( f2(6, 36));
    ...
    constexpr bool foo = cuda_for_dali::std::less<int> () (36, 36);
    static_assert ( !foo, "" );

    constexpr bool bar = cuda_for_dali::std::less<> () (36.0, 36);
    static_assert ( !bar, "" );
#endif
