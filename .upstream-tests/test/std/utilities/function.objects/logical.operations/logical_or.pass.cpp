...
int main(int, char**)
{
    typedef cuda_for_dali::std::logical_or<int> F;
    const F f = F();
    ...
    assert(!f(0, 0));
#if TEST_STD_VER > 11
    typedef cuda_for_dali::std::logical_or<> F2;
    const F2 f2 = F2();
    assert( f2(36, 36));
    assert( f2(36, 36L));
    assert( f2(36L, 36));
    ...
    constexpr bool foo = cuda_for_dali::std::logical_or<int> () (36, 36);
    static_assert ( foo, "" );

    constexpr bool bar = cuda_for_dali::std::logical_or<> () (36.0, 36);
    static_assert ( bar, "" );
#endif
