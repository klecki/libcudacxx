...
int main(int, char**)
{
    ...
    ASSERT_SAME_TYPE(bool, decltype(cuda_for_dali::std::declval<const month_weekday>().ok()));

    static_assert(!month_weekday{month{}, weekday_indexed{}}.ok(),                   "");
    static_assert( month_weekday{cuda_for_dali::std::chrono::May, weekday_indexed{Sunday, 2}}.ok(), "");

    assert(!(month_weekday(cuda_for_dali::std::chrono::April, weekday_indexed{Sunday, 0}).ok()));
    assert( (month_weekday{cuda_for_dali::std::chrono::March, weekday_indexed{Sunday, 1}}.ok()));
