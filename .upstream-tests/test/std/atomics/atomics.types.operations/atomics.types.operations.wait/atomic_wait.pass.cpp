...
template <class T, template<typename, typename> typename Selector, cuda_for_dali::thread_scope Scope>
struct TestFn {
  __host__ __device__
  void operator()() const {
    ...
    cuda_for_dali::std::atomic_wait(t, T(0));
#ifdef __CUDA_ARCH__
    }
    __syncthreads();
#endif
    ...
    auto agent_notify = LAMBDA (){
      cuda_for_dali::std::atomic_store(t, T(3));
      cuda_for_dali::std::atomic_notify_one(t);
    };

    auto agent_wait = LAMBDA (){
      cuda_for_dali::std::atomic_wait(t, T(1));
    };
    ...
    cuda_for_dali::std::atomic_wait(vt, T(1));
#ifdef __CUDA_ARCH__
    }
    __syncthreads();
#endif
    ...
    auto agent_notify_v = LAMBDA (){
      cuda_for_dali::std::atomic_store(vt, T(4));
      cuda_for_dali::std::atomic_notify_one(vt);
    };
    auto agent_wait_v = LAMBDA (){
      cuda_for_dali::std::atomic_wait(vt, T(2));
    };
