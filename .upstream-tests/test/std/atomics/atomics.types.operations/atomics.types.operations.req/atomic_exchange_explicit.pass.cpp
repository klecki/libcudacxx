...
struct TestFn {
  ...
  void operator()() const {
    ...
    assert(cuda_for_dali::std::atomic_exchange_explicit(&t, T(2), cuda_for_dali::std::memory_order_seq_cst)
           == T(1));
    assert(t == T(2));
    Selector<volatile A, constructor_initializer> vsel;
    volatile A & vt = *vsel.construct();
    cuda_for_dali::std::atomic_init(&vt, T(3));
    assert(cuda_for_dali::std::atomic_exchange_explicit(&vt, T(4), cuda_for_dali::std::memory_order_seq_cst)
           == T(3));
    assert(vt == T(4));
  }
};
