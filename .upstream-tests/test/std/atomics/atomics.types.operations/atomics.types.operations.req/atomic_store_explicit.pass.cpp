...
struct TestFn {
  ...
  void operator()() const {
    ...
    A & t = *sel.construct();
    cuda_for_dali::std::atomic_store_explicit(&t, T(1), cuda_for_dali::std::memory_order_seq_cst);
    assert(t == T(1));
    Selector<volatile A, constructor_initializer> vsel;
    volatile A & vt = *vsel.construct();
    cuda_for_dali::std::atomic_store_explicit(&vt, T(2), cuda_for_dali::std::memory_order_seq_cst);
    assert(vt == T(2));
  }
};
