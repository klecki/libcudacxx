...
__host__ __device__ void test() {
  ...
  NV_DISPATCH_TARGET(
    ...
    NV_IS_DEVICE, (
      ...
      static_assert(nv_for_dali::target::detail::toint(NV_TARGET_MINIMUM_SM_SELECTOR) == (__CUDA_MINIMUM_ARCH__ / 10), "arch mismatch");
    )
  )
}
...
void test() {
  ...
  NV_DISPATCH_TARGET(
    ...
    NV_IS_DEVICE, (
      static_assert(NV_TARGET_MINIMUM_SM_INTEGER == (__CUDA_ARCH__ / 10), "arch mismatch");
      static_assert(nv_for_dali::target::detail::toint(NV_TARGET_MINIMUM_SM_SELECTOR) == (__CUDA_ARCH__ / 10), "arch mismatch");
      static_assert(__CUDA_MINIMUM_ARCH__ == __CUDA_ARCH__, "arch mismatch");
    )
  )
}
