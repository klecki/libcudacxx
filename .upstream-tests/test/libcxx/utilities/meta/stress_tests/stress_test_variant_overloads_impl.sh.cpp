...
#include <cuda_for_dali/std/variant>
...
struct MakeOverloads<cuda_for_dali::std::__tuple_indices<_Idx...> > {
...
using Overloads = typename MakeOverloads<
    cuda_for_dali::std::__make_indices_imp<sizeof...(Types), 0> >::template Apply<Types...>;
...
namespace variant_impl {
  template <class ...Types>
  using Overloads = cuda_for_dali::std::__variant_detail::_MakeOverloads<Types...>;
...
void fn2(typename cuda_for_dali::std::invoke_result_t<T1, int, int>::type x) { DoNotOptimize(&x); }
