...
// cuda_for_dali::std::atomic when used with __attribute__((vector(X))).
...
template <typename T> struct atomic_test : public cuda_for_dali::std::__atomic_base<T> {
  atomic_test() {
...
#define CHECK_ALIGNMENT(T)                                                     \
  ...
  CHECK_ALIGNMENT(cuda_for_dali::std::nullptr_t);
